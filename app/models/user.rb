...
class User < ApplicationRecord
  ...
  has_many :photos, class: "Photo", foreign_key: "owner_id"

  # User#likes: returns rows from the likes table associated to this user by the fan_id column
  has_many :likes, class:"Like", foreign_key: "fan_id"
  ...
  has_many :accepted_sent_follow_requests, -> { where(status: 'accepted') }, class: "FollowRequest", foreign_key: "sender_id"
  ...
  has_many :accepted_received_follow_requests, -> { where(status: 'accepted') }, class: "FollowRequest", foreign_key: "recipient_id"
